...
const handler: Handler = async (event, context) => {
  const data = {
    user: {
      id: 'fakeidlocal-1',
      app_metadata: { roles: [], provider: 'fake' },
      user_metadata: { avatar_url: 'fake url', full_name: 'test-user' },
    },
  };
  event = { body: JSON.stringify({ event: 'REGISTER', ...data }) } as any;
...
if (require.main === module) {
  ...
  const event = { body: JSON.stringify({ event: 'REGISTER', ...data }) };
}
